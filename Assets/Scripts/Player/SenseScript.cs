...
/// <summary>
/// Controls the spider sensing mechanic. This script is attached to the player's sense object
/// and is responsible for handling its position and rotation based on the player's movement state and
/// direction.
/// </summary>
...
public class SenseScript : MonoBehaviour
{
    ...
    /// <summary>
    /// Calculates and updates the sense object's position and rotation based on the player's current movement state and
    /// animation progress.
    /// </summary>
    private void CalculateSensePosition()
    {
        if (player.sprite.flipX) dir = -1f; else dir = 1f;
        mstate = player.AnimationDriver.GetMovementState();
        float normalizedTime = player.AnimationDriver.CurrentState.normalizedTime % 1f;
    ...
    /// <summary>
    /// Converts a local 2D offset to a world position relative to the specified player's transform, preserving the
    /// current object's Z position.
    /// </summary>
    /// <param name="playerTransform">The transform representing the player's coordinate space in which the offset is defined. Cannot be null.</param>
    /// <param name="offset">The 2D offset, in the local space of the specified player transform, to convert to world coordinates.</param>
    /// <returns>A Vector3 representing the world position corresponding to the given offset, with the Z coordinate set to the
    /// current object's Z position.</returns>
