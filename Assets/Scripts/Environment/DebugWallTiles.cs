...
/// <summary>
/// For debugging wall tile boundaries in editor. Helpful if noticing weird wall interactions or incorrect zip indicator placements.
/// </summary>
...
public class TilemapCollisionGizmo : MonoBehaviour
{
    ...
    /// <summary>
    /// Calculates the bounds of tiles visible to the specified camera on the tilemap's plane.
    /// </summary>
    /// <param name="cam">The camera used to determine the visible area of the tilemap. Must not be null.</param>
    /// <returns>A BoundsInt representing the rectangular region of tile cells that are visible to the camera, including any
    /// additional padding cells.</returns>
    ...
    /// <summary>
    /// Calculates the world-space point where a ray from the specified camera, passing through a given viewport
    /// coordinate, intersects a plane.
    /// </summary>
    /// <param name="cam">The camera used to generate the ray from the viewport point.</param>
    /// <param name="plane">The plane in world space to intersect with the ray.</param>
    /// <param name="viewportPoint">The viewport coordinates (normalized between 0 and 1) representing the point on the camera's viewport through which the ray is cast.</param>
    /// <returns>The world-space position where the ray from the camera through the viewport point intersects the plane. If the
    /// ray is parallel to the plane, returns a fallback world position at the plane's distance along the ray direction.</returns>
