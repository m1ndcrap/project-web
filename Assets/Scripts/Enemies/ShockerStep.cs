...
/// <summary>
/// Represents the Shocker enemy boss, managing its state machine, combat behavior, environment interactions, and barrier
/// logic within the game. Implements enemy barrier functionality to control player movement collisions during encounters.
/// </summary>
...
public class ShockerStep : MonoBehaviour, IEnemyBarrier
{
    ...
    /// <summary>
    /// This enemy's high-level AI state, driving the switch statement in <see cref="Update"/>.
    /// <list type="bullet">
    /// <item><description><b>chase</b>: a scripted, level-specific pursuit sequence that plays out once when the player first draws this enemy's attention. See the case's own comment for why it's hardcoded to specific level coordinates.</description></item>
    /// <item><description><b>engaged</b>: standard ground combat once the chase sequence ends, closes distance for a melee attack, or backs off to throw a ranged shockwave beam.</description></item>
    ...
    /// <item><description><b>getting_hit</b>: playing a hit reaction before returning to engaged.</description></item>
    /// <item><description><b>evade</b>: dashing a short distance away (bounded by the arena), possibly rushing back into an attack afterward.</description></item>
    /// <item><description><b>death</b>: defeated, stops colliding with the player and settles under gravity.</description></item>
    /// </list>
    /// </summary>
    ...
    [Tooltip("This enemy's own health bar UI. Assign the correct health bar in the scene directly, now that HealthBar is a shared, general-purpose widget, an automatic scene search could just as easily find a different object's health bar.")]
    ...
    /// <summary>
    /// Plays one of this enemy's three melee swipe sound variations at random.
    /// </summary>
    ...
    /// <summary>
    /// True while this enemy should physically block the player as a solid barrier (part of <see cref="IEnemyBarrier"/>).
    /// False outside engaged/evade, so the player can pass through while it's chasing, attacking, hurt, or dead.
    /// </summary>
    ...
    /// <summary>
    /// This enemy's collider, used by the player's barrier-blocking logic (part of <see cref="IEnemyBarrier"/>).
    /// </summary>
    ...
    void Update()
    {
        if (forceRunAway)
        {
            ...
            rb.gravityScale = 1f;
            animDriver.Speed = 1f;
            animDriver.SetMovementState((int)MovementState.sprinting);
            return;
        }
            ...
            else
            {
                if (sState != ShockerState.death && sState != ShockerState.chase)
                {
                    if (Vector3.Distance(player.transform.position, transform.position) <= 0.8f)
                    {
                        ...
                        audioController.Play(sndShockwaveBlast);
                        PlayAttackSounds();
                        sState = ShockerState.attack;
            ...
            else
            {
                if (sState == ShockerState.engaged)
                {
                    if (throwing)
                    {
                        ...
                        audioController.PlayRandom(sndShoot, sndShoot2);
                        threw = true;
                    }
                }
        ...
        if (sState != ShockerState.death && sState != ShockerState.chase)
        {
            ...
            {
                audioController.PlayRandom(sndWin1, sndWin2);
                win = true;
            }
        }
        ...
        switch (sState)
        {
            // A scripted, phase-by-phase pursuit sequence hardcoded to one level's geometry (exact world coordinates gate each phase transition)
            case ShockerState.chase:
                {
                    ...
                    if (phase == 0)
                    {
                        if (distanceFromPlayer < 2f)
                        {
                            ...
                            player.trigger = true;
                            player.alarm4 = 60;
                            audioController.Play(sndShockerIntro);
                    ...
                    if (phase == 1)
                    {
                        ...
                        else
                        {
                            ...
                            if (distanceFromPlayer < 1f)
                            {
                                ...
                                player.trigger = true;
                                player.alarm4 = 60;
                                audioController.PlayRandomWithSilenceChance(sndShockerMove);
                                bgmController.GetComponent<BGMController>().intensity = 1;
                            }
                        }
            ...
            // Standard ground combat: closes distance for a melee attack when close, or throws a ranged shockwave beam when far enough away and off cooldown.
            case ShockerState.engaged:
                {
                    ...
                    if (threw)
                    {
                        ...
                        if (stateInfo.normalizedTime >= 0.5f && stateInfo.normalizedTime <= 0.53f && FindObjectsOfType<PumpkinProjectile>().Length == 0)
                        {
                            ...
                            audioController.Play(sndShockwaveBeam);
                            player.trigger = true;
                            player.alarm4 = 60;
                            player.isEnemyAttacking = true;
                        }
            ...
            // Mid-punch/kick: closes the last bit of distance during the animation's early frames, then returns to engaged once it finishes, or triggers an evade instead
            ...
            // Reacting to a hit, returns to engaged once the animation finishes, possibly triggering a forced evade (see TryForceEvadeAfterHit).
            ...
            // Dashes a short distance away (started via StartEvasion), stopping early at the arena bounds rather than running past them. Once the dash finishes, either rushes straight into an attack (if evadeWillRush is true and the player is close enough) or settles back into engaged.
            ...
            // Defeated: stops colliding with the player and settles to the ground under normal gravity.
    ...
    private void UpdateAnimationState()
    {
        ...
        if (sState == ShockerState.evade)
        {
            ...
            AnimatorStateInfo evadeStateInfo = animDriver.CurrentState;
    ...
    /// <summary>
    /// Begins the evade state: picks a direction away from the player and a random evade duration,
    /// and rolls whether this evade will end in a rush-attack. Unlike RobotStep's evasion, this
    /// doesn't check hazards or ledges first, the evade dash itself is clamped to the arena bounds
    /// in the evade case below, so there's no separate direction-safety check needed here.
    /// </summary>
    ...
    /// <summary>
    /// Rolls a chance to force this enemy into evasion right after taking a hit, guaranteed after 2 or more hits in a row (hitStreak), a coin flip otherwise.
    /// </summary>
    ...
    /// <summary>
    /// Called when the player's attack (or counter) connects with this enemy. Applies damage, picks
    /// a hurt reaction, plays the matching hit sound and FX, and updates the health bar.
    /// Combat-recovery timing (<c>alarm4</c>) is re-rolled here too, so this enemy doesn't
    /// immediately re-attack right after being hit.
    /// </summary>
    /// <param name="target">This enemy itself, passed through from the player's hit-resolution event.</param>
    public void OnPlayerHit(ShockerStep target)
    {
        ...
        if (sState == ShockerState.engaged)
        {
            ...
            audioController.PlayRandom(sndHit, sndHit2, sndHit3, sndHit4);
    ...
    /// <summary>
    /// Called from an Animation Event on this enemy's attack impact frame. Deals damage to the player
    /// if they're still within range by that point in the animation.
    /// </summary>
